...
REMOTE_MANIFEST_URL="https://storage.googleapis.com/flutter_infra_release/releases/${MANIFEST_JSON_PATH}"
LOCAL_MANIFEST_PATH="$HOME/.github/actions/$MANIFEST_JSON_PATH"  # Change this path as needed
...
download_manifest_if_not_exists() {
    if [[ ! -f "$LOCAL_MANIFEST_PATH" ]]; then
        ...
        curl --connect-timeout 15 --retry 5 -o "$LOCAL_MANIFEST_PATH" "$REMOTE_MANIFEST_URL"
    fi
}
